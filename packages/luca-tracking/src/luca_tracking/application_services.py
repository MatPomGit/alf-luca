...
def _resolve_track_output_paths(source_label: str, output_csv: str, trajectory_png: Optional[str], report_csv: Optional[str], report_pdf: Optional[str]) -> tuple[str, str, str, str]:
    ...
    resolved_output_csv = resolve_output_path(output_csv_value)
    resolved_trajectory_png = resolve_output_path(trajectory_png or f"{base}_trajectory.png")
    resolved_report_csv = resolve_output_path(report_csv or f"{base}_report.csv")
    resolved_report_pdf = resolve_output_path(report_pdf or f"{base}_report.pdf")
...
def run_tracking(args: Namespace) -> None:
    ...
    if getattr(args, "config", None):
        ...
        run_config.eval.trajectory_png = resolve_output_path(with_default(run_config.eval.trajectory_png, f"{base}_trajectory.png"))
    ...
    if getattr(args, "video", None):
        video = resolve_analysis_input(args.video)
        source_label = video
...
def run_compare(reference: str, candidate: str, output_csv: str, report_pdf: str | None = None) -> None:
    ...
    ensure_output_dir()
    compare_csv(
        resolve_analysis_input(reference),
        resolve_analysis_input(candidate),
        resolve_output_path(output_csv),
        resolve_output_path(report_pdf) if report_pdf else None,
    )
